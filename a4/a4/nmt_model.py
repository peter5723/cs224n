...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #enc_hiddens = pad_packed_sequence(self.encoder(X_pack)[0])[0].permute(enc_hiddens, (1, 0, 2))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            state, o_t, cell = self.step(Ybar_t, dec_state=dec_state,
                                                enc_hiddens=enc_hiddens, enc_hiddens_proj=enc_hiddens_proj,
                                                enc_masks=enc_masks)
        ...
        #decode是一整个更新过程, 在decode函数中包括使用decoder(也就是LSTM)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        enc_hiddens_proj_T = torch.permute(enc_hiddens_proj, (0, 2, 1))
        e_t = torch.bmm(dec_hidden, enc_hiddens_proj_T)
        ...
        #dec_hidden = dec_hidden.squeeze(1) #前面unsqueeze过, 现在要squeeze回来
