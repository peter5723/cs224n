...
def minibatch_parse(sentences, model, batch_size):
    ...
    """partial_parses = []
    for st in sentences:
        temp_partial_parses = PartialParse(st)
        partial_parses.append(temp_partial_parses)
        ##要会用列表循环式, 比for省力
    """
    partial_parses = [PartialParse(st) for st in sentences]
    #initialize partial_parses
    unfinished_parses = partial_parses[:]
    #initialize unfinished_parses
    while unfinished_parses:
        batch_size_parses = unfinished_parses[0:batch_size]
        #python列表切片a[index1:index2], 即使后面的索引大于len(list)也没关系的, 也就是索引到min(len(list), index2)
        transitions = model.predict(batch_size_parses)
        """
        for parse, tran in zip(unfinished_parses, transitions):
            parse.parse_step(tran)
            if len(parse.buffer) == 0 and len(parse.stack) == 1:
                unfinished_parses.remove(parse)
        ##不能这样写, 如果这样写的话, python是动态数组, 下标对不上
        """
        for parse, tran in zip(unfinished_parses, transitions):
            parse.parse_step(tran)
        unfinished_parses = [parse for parse in unfinished_parses if not (len(parse.stack)==1 and len(parse.buffer)==0)]
    ###dependencies = [partial_parses[i].dependencies for i in range(len(partial_parses)-1)]错误,不知道为啥
    dependencies = [partial_parse.dependencies for partial_parse in partial_parses]
    ##[:], zip都是shallow copy, 简单的理解和指针类似, 故list中对象改了, 其他地方也改
    ##remove() /pop()只是删除"指针",然而del()是删除实体,所以慎用del
